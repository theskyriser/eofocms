...
import {MdPersonOutline, MdInfoOutline, MdOutlineGrade, MdPersonAddAlt, MdPersonRemoveAlt1, MdOutlineSchedule, MdAttachMoney, MdMoney, MdOutlineQueue } from "react-icons/md";
...
const ManageTeachers = () => {
    ...
    const thisTeacher = teachers.filter((teacher) => teacher._id === selectedTeacher)[0]
        ...
        <div className='p-4'>
            <div className=' bg-main-bg rounded-lg shadow-lg p-4'>
                    ...
                    <div className='col-span-3 bg-white rounded-lg shadow-lg p-5 flex gap-4'>
                        ...
                        <div className='text-center flex flex-col justify-between'>
                            <div>
                                ...
                                <h2 className='mt-1 font-semibold text-lg'> {thisTeacher?.position ? thisTeacher?.position : 'No record'} </h2>
                            </div>
                        ...
                        <div className='text-lg grid grid-cols-4 gap-4 w-full bg-main-bg rounded-lg shadow-lg p-2'>
                            ...
                            <div className='col-span-3 col-start-3 font-small text-left'>
                                <p> {thisTeacher?.firstName && thisTeacher?.lastName ? thisTeacher?.firstName + ' ' + thisTeacher?.lastName : 'No record'} </p>
                                <p> {thisTeacher?.email ? thisTeacher?.email : 'No record'} </p>
                                <p> {thisTeacher?.phone ? thisTeacher?.phone : 'No record'} </p>
                                <p> {thisTeacher?.address ? thisTeacher?.address : 'No record'} </p>
                                <p> {thisTeacher?.state ? thisTeacher?.state : 'No record'} </p>
                                <p> {thisTeacher?.code ? thisTeacher?.code : 'No record'} </p>
                                <p> {thisTeacher?.numberOfClasses ? thisTeacher?.numberOfClasses : 'No record'} </p>
                            </div>
                        </div>
                    </div>

                    <div className='col-start-4 col-span-5'>
                        <GridComponent selectedRowIndex={0} dataSource={teachers} rowSelected={(rowData) => handleRowSelect(rowData)} rowDeselected={() => handleRowDeselect()}>
                            <ColumnsDirective>
                ...
                <div className='col-start-1 col-span-1 shadow-lg bg-white mt-5 p-4'>
                        ...
                        <button class="py-2 px-4 rounded-md flex justify-between mb-2 w-full hover:shadow-md" onClick={() => handleAdd()} style={{backgroundColor: currentColor, color: 'white'}}>
                        ...
                        <button class="py-2 px-4 rounded-md flex justify-between mb-2 w-full hover:shadow-md" onClick={() => handleEdit()} style={{backgroundColor: currentColor, color: 'white'}}>
                    ...
                    <div className='col-start-2 col-span-1 shadow-lg bg-white mt-5 p-4'>
                        <h2 className='text-xl font-semibold mb-3'> Teacher's Classes </h2>

                        

                        <button class="py-2 px-4 rounded-md flex justify-between mb-2 w-full hover:shadow-md" onClick={() => handleOpenSchedule()} style={{backgroundColor: currentColor, color: 'white'}}>
                    ...
                    <div className='col-start-3 col-span-1 shadow-lg bg-white mt-5 p-4'>
                        ...
                        <button class="py-2 px-4 rounded-md flex justify-between mb-2 w-full hover:shadow-md" onClick={() => setActivePayroll(true)} style={{backgroundColor: currentColor, color: 'white'}}>
                        ...
                        <button class="py-2 px-4 rounded-md flex justify-between mb-2 w-full hover:shadow-md" onClick={() => {}} style={{backgroundColor: currentColor, color: 'white'}}>
            ...
            <AnimatePresence intial={false} exitBeforeEnter={true} onExitComplete={(() => null)}>
                {activeAddTeacher &&
                    <AddTeacher setActiveAddTeacher={setActiveAddTeacher} activeAddTeacher={activeAddTeacher} currentEdit={currentEdit} selectedTeacher={selectedTeacher}/>     
                }

                {activeSchedule &&
                    <ExpandTeacher selectedTeacher={selectedTeacher} setActiveSchedule={setActiveSchedule} currentColor={currentColor}/>
                }

                {activePayroll &&
                    <PayrollComp selectedTeacher={selectedTeacher} setActivePayroll={setActivePayroll}/>
                }

                {activeCredentials &&
                    <ChangeCredentials id={selectedTeacher} type={"TEACHER"} setActiveCredentials={setActiveCredentials}/>
                }
            </AnimatePresence>
        </div>
  )
