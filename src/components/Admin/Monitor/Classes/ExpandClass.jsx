...
const ExpandClass = ({setActiveExpand, selectedClass, currentColor}) => {
  let calendar;
  const dispatch = useDispatch()
  const classes = useSelector(state => state.classes)
  const currentClass = classes.filter((item) => item._id === selectedClass)[0]
  ...
  const handleCalendarChange = () => {
    ...
    if (dates) {
      ...
      dispatch(getSessionsForCalendar(`searchDateStart=${firstDate}&searchDateEnd=${lastDate}&type=class&id=${selectedClass}`))
    }
}
