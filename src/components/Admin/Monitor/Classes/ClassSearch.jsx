  ...
  const handleFilterSubmit = (type, data) => {
    switch (type) {
      case "teacher":
        ...
        setFilterDisplay({
          ...filterDisplay,
          teacher: { firstName: data.firstName, lastName: data.lastName },
        });
        setAddTeacher(false);
        break;
      ...
      case "coor":
        ...
        setFilterDisplay({
          ...filterDisplay,
          coor: { firstName: data.firstName, lastName: data.lastName },
        });
        setAddCoor(false);
        break;
            ...
            <form className="grid grid-cols-4 gap-4">
              <div>
                <input
                  ...
                  className="h-full text-md p-1 border-1 border-color rounded-md"
                  onChange={(e) => {
                    setSearchParams({ ...searchParams, group: e.target.value });
                  }}
                />
              </div>
              ...
              <div className="relative col-start-2 col-span-1">
                <select
                  ...
                  className="rounded-md appearance-none block w-full text-gray-700 border-1 border-color p-2 leading-tight focus:outline-none focus:bg-white focus:border-gray-500"
                >
              ...
              <div className="col-start-1 col-span-1 text-center border-color border-1 p-2 rounded-md">
                ...
                <p className="mt-3">
                  {filterDisplay.teacher
                    ? filterDisplay?.teacher?.firstName +
                      " " +
                      filterDisplay?.teacher?.lastName
                    : "Not Selected"}
                </p>
              </div>
              ...
              <div className="col-start-2 col-span-1 text-center border-color border-1 p-2 rounded-md">
                ...
                <p className="mt-3">
                  {filterDisplay.coor
                    ? filterDisplay?.coor?.firstName +
                      " " +
                      filterDisplay?.coor?.lastName
                    : "Not Selected"}
                </p>
              </div>
              ...
              <div className="col-start-3 col-span-1 text-center border-color border-1 p-2 rounded-md">
                ...
                <p className="mt-3">
                  {filterDisplay.student
                    ? filterDisplay?.student?.firstName +
                      " " +
                      filterDisplay?.student?.lastName
                    : "Not Selected"}
                </p>
              </div>
              ...
              <div className="col-start-4 col-span-1 rounded-md flex flex-col-reverse">
                <button
                  type="button"
                  className={`text-sm p-2 border-color border-1 rounded-md hover:shadow-lg text-semibold text-white`}
